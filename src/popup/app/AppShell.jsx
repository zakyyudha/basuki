...
function Toggle({ checked, onChange, label }) {
  return (
    <button type="button" role="switch" aria-checked={checked} aria-label={label}
      onClick={() => onChange(!checked)} className={`toggle${checked ? ' on' : ''}`}>
...
function RedirectsPanel({ lang, rules, onToggle, onAdd, onEdit, onDelete }) {
  ...
  return (
    <PanelShell label={t('panel_redirects', lang)}
      ...
      onAdd={onAdd} addLabel={t('add_rule', lang)}>
      {rules.length === 0 && <button className="empty-state" onClick={onAdd}>{t('add_rule', lang)}</button>}
      {rules.map(r => (
...
function InterceptsPanel({ lang, rules, onToggle, onAdd, onEdit, onDelete }) {
  ...
  return (
    <PanelShell label={t('panel_intercepts', lang)}
      counter={`${String(active).padStart(2,'0')}/${String(rules.length).padStart(2,'0')}`}
      onAdd={onAdd} addLabel={t('add_mock', lang)}>
      {rules.length === 0 && <button className="empty-state" onClick={onAdd}>{t('add_mock', lang)}</button>}
      {rules.map(r => (
        <article key={r.id} className="rule-card">
          <div className="rule-card__head">
            <div>
...
function SessionsPanel({ lang, sessions, onLaunch, onClose, onAdd, onEdit, onDelete }) {
  ...
  return (
    <PanelShell label={t('panel_sessions', lang)}
      ...
      onAdd={onAdd} addLabel={t('add_session', lang)}>
      {sessions.length === 0 && <button className="empty-state" onClick={onAdd}>{t('add_session', lang)}</button>}
      {sessions.map(s => (
...
export function AppShell({ state, store, onLanguageChange }) {
  ...
  // Local CRUD state (seeded from storage state when available)
  const [redirects, setRedirects] = useState(() => state?.redirectConfigs || [])
  const [intercepts, setIntercepts] = useState(() => state?.interceptConfigs || [])
  const [sessions, setSessions] = useState(() => state?.isolatedTabs || [])

  // Editor state: { kind, id, isNew } | null
  ...
  const addRedirect = () => {
    const id = `r${Date.now()}`
    setRedirects(prev => [...prev, { id, name: 'New Redirect', from: 'https://example.com/*', to: 'http://localhost:3000', enabled: false, hits: 0 }])
    setEditor({ kind: 'redirect', id, isNew: true })
  }
  const saveRedirect = (next) => {
    setRedirects(prev => prev.map(r => r.id === next.id ? next : r))
    setEditor(null)
  }
  ...
  const toggleRedirect = (id) => setRedirects(prev => prev.map(r => r.id === id ? { ...r, enabled: !r.enabled } : r))

  // ── Intercept CRUD ────────────────────────────────────────────────────────
  const addIntercept = () => {
    const id = `i${Date.now()}`
    setIntercepts(prev => [...prev, { id, name: 'New Mock', method: 'GET', pattern: '/api/example', status: 200, body: '{}', enabled: false }])
    setEditor({ kind: 'intercept', id, isNew: true })
  }
  const saveIntercept = (next) => {
    setIntercepts(prev => prev.map(r => r.id === next.id ? next : r))
    setEditor(null)
  }
  const deleteIntercept = (id) => {
    setIntercepts(prev => prev.filter(r => r.id !== id))
    setEditor(null)
  }
  const toggleIntercept = (id) => setIntercepts(prev => prev.map(r => r.id === id ? { ...r, enabled: !r.enabled } : r))

  // ── Session CRUD ──────────────────────────────────────────────────────────
  const addSession = () => {
    const id = `s${Date.now()}`
    setSessions(prev => [...prev, { id, name: 'New Session', origin: 'example.com', cleanState: true, userAgent: 'Desktop/Chrome', active: false }])
    setEditor({ kind: 'session', id, isNew: true })
  }
